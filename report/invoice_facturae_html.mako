    ...
    %for o in objects :
                ...
                <tr>
                    <table width="100%" class="basic_table" style="font-size:9; border:1.5px solid grey;">
                        ...
                        <tr>
                            ...
                            <td class="cliente"><b>${_('Método de pago')}</b></td><td class="cliente">${ method or '' }</td>
        ...
        <table class="basic_table" style="color:#121212">
            ...
            %for dict in range(0,len(dict_lines)):
                ...
                %endif
                    ...
                    <td width="9%" class="number_td"><% vuni = dict_lines[dict]['@valorUnitario'] %>${formatLang(float(vuni)) or '0.0'}</td>
                    <td width="15%" class="number_td"><% imp = dict_lines[dict]['@importe'] %>${formatLang(float(imp)) or '0.0'}</td>
                    </tr>
                <%row_count+=1%>
            %endfor
        </table>
        <table align="right" width="30%" style="border-collapse:collapse">
            <tr>
                ...
                <td align="right" class="total_td">${ o.invoice_source_id.currency_id.symbol or '' } ${ round(float(dict_data['@subTotal']),4) or ''|entity}</td>
            </tr>
            <% desc_amount = float(dict_data['@descuento']) %>
            %if desc_amount > 0:
            <tr>
                <td class="total_td">${_("Descuento:")}</td>
                <td align="right" class="total_td">$ ${formatLang(float( dict_data['@descuento'])) or ''|entity}</td>
            </tr>
            %endif
                ...
                %if imp_amount > 0:
                    <tr>
                        ...
                        <td class="tax_td" align="right">
                            $ ${formatLang(float(imp_amount)) or '0.0'|entity}
                        </td>
                    </tr>
                %endif
            %endfor
                ...
                %for ret in range(0,len(dict_ret)):
                    ...
                    %if ret_amount > 0:
                        <tr>
                            ...
                            <td class="tax_td" align="right">
                                $ ${formatLang(float( ret_amount)) or '' | entity }
                            </td>
                        </tr>
                    %endif
                %endfor
            ...
            <tr align="left">
                ...
                <td class="total_td" align="right"><b>${ o.invoice_source_id.currency_id.symbol or '' } ${formatLang(float( dict_data['@total'])) or ''|entity}</b></td>
            </tr>
        </table>
        <br clear="all" />
        <table class="basic_table">
        ...
        <table class="basic_table">
            ...
            <tr>
                <td style="text-align: left;vertical-align: middle;font-size:15px; font-family: Helvetica;">
                    <i>${ o.invoice_source_id.comment or '' }</i>
                </td>
            </tr>         
        </table>
        <br clear="all"/>
        ...
        <table class="basic_table" rules="cols" style="border:1.5px solid grey;">
                ...
                <tr>
                    ...
                    <td class="center_td">${ method or '' }</td>
